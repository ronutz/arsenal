...
import { prose, proseList, prosePairs } from "@/lib/roles-prose";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  /* THE TITLE AND DESCRIPTION IN THE READER'S LANGUAGE (2026-08-16).
     These read `role.whatItIs` straight out of roles.ts, so a Portuguese page
     carried an English <title> and <meta description> above Portuguese body
     copy - half and half, in the two strings a search engine reads first.

     The role's own NAME is authored too: "Technical instructor" is not what a
     Portuguese reader calls the job. */
  const messages = (await getMessages({ locale })) as { roles?: unknown };
  const m = messages.roles;
  const title = prose(m, role.slug, "title", role.title);
  const what = prose(m, role.slug, "whatItIs", role.whatItIs);
  return { title: `${title} - ${t("title")}`, description: what.slice(0, 155) };
}
...
export default async function RolePage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  /* THE AUTHORED PROSE, in the reader's language (2026-08-16).
     roles.ts holds the English and the structure; the message files hold both
     languages. A field falls back to the English until its Portuguese is
     written - see src/lib/roles-prose.ts and check-roles-bilingual. */
  const messages = (await getMessages({ locale })) as { roles?: unknown };
  const m = messages.roles;
  const say = (name: string, english: string) => prose(m, role.slug, name, english);
  const sayList = (name: string, english: readonly string[]) =>
    proseList(m, role.slug, name, english);
  ...
  const citation =
    role.provenance.kind === "documented"
      ...
      : `${t(`provenance.${role.provenance.kind}`)} - ${say("provenanceWhere", role.provenance.where ?? "")}, ${role.provenance.when}`;
  ...
  return (
    <>
        ...
        <div className="container article-container">
            <Breadcrumbs
              ...
              items={[
                ...
                { label: say("title", role.title) },
              ]}
            />
          <p className="hero-eyebrow">
            <Link href="/roles">{t("eyebrow")}</Link> {"\u00b7"} {t(`groups.${role.group}.title`)}
          </p>
          <h1 className="article-title">{say("title", role.title)}</h1>
          ...
          <p className="article-summary">{say("whatItIs", role.whatItIs)}</p>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container article-container">
          <List heading={t("sections.theDay")} items={sayList("theDay", role.theDay)} />
        </div>
      </section>
      ...
      <section className="section section-accent">
        <div className="container article-container">
          ...
          <List heading={t("sections.accountableFor")} items={sayList("accountableFor", role.accountableFor)} />
          <List heading={t("sections.measuredOn")} items={sayList("measuredOn", role.measuredOn)} />
        </div>
      </section>
      ...
      <section className="section">
        <div className="container article-container">
          ...
          <dl className="role-exchange">
            {prosePairs(m, role.slug, "receivesFrom", role.receivesFrom).map((x, i) => (
          ...
          <dl className="role-exchange">
            {prosePairs(m, role.slug, "serves", role.serves).map((x, i) => (
              <div className="role-exchange-item" key={i}>
                <dt className="role-exchange-who">{x.who}</dt>
                <dd className="role-exchange-what">{x.what}</dd>
              </div>
      ...
      <section className="section section-accent">
        <div className="container article-container">
          <List heading={t("sections.stakeholders")} items={sayList("stakeholders", role.stakeholders)} />
          <List heading={t("sections.requirements")} items={sayList("requirements", role.requirements)} />
        </div>
      </section>
      ...
      <section className="section">
        <div className="container article-container">
          ...
          <p className="role-turns-on">{say("turnsOn", role.turnsOn)}</p>
        </div>
      </section>
