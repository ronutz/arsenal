// ============================================================================
// src/content/bofh/excuses.ts
// ----------------------------------------------------------------------------
// THE BOFH EXCUSE CORPUS. Served, one at random per visit, on the 404 page:
// when a page cannot be found, the Bastard Operator From Hell (Simon
// Travaglia's legendary sysadmin, Usenet 1992 onward) has already closed the
// ticket and filed the cause.
//
// SOURCE (verbatim, fetched 04/07/2026): excuses.txt from the gist PRIME
// supplied, https://gist.github.com/chuckwagoncomputing/ff980f81482b5894c824
// (483 lines; the classic sysadmin "excuse of the day" list as it circulates
// in fortune files and BOFH-style excuse servers). Kept byte-for-byte,
// INCLUDING the source's own typos ("techtonic stress", "user to computer
// ration", "connot compute") - they are part of the artifact. The excuses
// stay in English on every locale: this is sysadmin folklore, and
// translating it would kill the joke.
//
// EXCLUDED (brand safety, ANVIL editorial call pending PRIME ratification):
// exactly two entries are withheld from the SERVED list - one for profanity,
// one for a September-11 reference - both unfit for a professional site's
// random surface. They remain in RAW below, unmodified, for corpus fidelity;
// to restore either, delete it from EXCLUDED.
// ============================================================================
...
insufficient data   connot compute
Please press all keys simultaneously to continue working.
It is pitch black. You are likely to be eaten by a grue.
String for throttle cable broke
Someone is using the power supply cable to pull out their stuck Chevy
Packet is being inspected by the ATF.
PETA thought you were mistreating the computer so they did rm /usr
File handler called in sick
File handler kidnapped by PETA
Your mouse has moved. Please restart Windows for the change to take effect.
`;

// The two withheld entries (see header). Matched verbatim against RAW.
...
/**
 * Every excuse in the corpus, minus the two EXCLUDED entries: trimmed, empty
 * lines dropped. 481 served out of 483 in the source.
 */
export const BOFH_EXCUSES: readonly string[] = RAW.split("\n")
  .map((line) => line.trim())
