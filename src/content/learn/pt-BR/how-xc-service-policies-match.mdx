...
concepts: ["f5 xc", "service policy", "distributed cloud", "waf", "access policy", "f5"]
...
## A ordem de casamento é onde uma política se torna auditável ou não

Uma requisição é avaliada contra regras numa ordem, e **a ordem é a parte que o leitor precisa reconstruir de cabeça** — exatamente onde a revisão falha. Uma política correta que exige simulação para ser entendida será mantida por adivinhação.

Isso dá um teste prático para qualquer regra que você acrescente: **alguém lendo a lista, de cima para baixo, consegue prever o que isto faz com uma requisição?** Se a resposta exige um traço de execução, a política está fazendo algo para o qual uma política é a ferramenta errada.

**Mantenha pequeno o número de regras que podem casar com uma mesma requisição.** Não por desempenho — pela propriedade muito mais cara de alguém precisar alterar isso sob pressão e acertar de primeira.
