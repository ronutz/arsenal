...
## O +trace não testa o seu caminho de resolução

`+trace` é a ferramenta certa para ler uma cadeia de delegação e a errada para provar que a resolução de nomes funciona, porque **ele ignora seu resolvedor de propósito.** Ele itera desde a raiz, perguntando a cada servidor por vez.

Então o `+trace` pode ter sucesso lindamente enquanto toda aplicação do host continua falhando, porque o que está quebrado é o resolvedor que o `+trace` nunca usou — seus forwarders, seu cache, suas regras de acesso, ou o caminho até ele.

**Use `+trace` para descobrir onde uma delegação está errada. Use uma consulta comum para descobrir se a resolução funciona.** São perguntas diferentes, e a segunda costuma ser a que está sendo feita.
