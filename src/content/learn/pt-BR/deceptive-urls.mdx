...
## O parser e o leitor discordam, e só um deles é autoritativo

Toda técnica de URL enganosa explora a mesma brecha: **um humano lê da esquerda para a direita e atribui sentido ao que parece familiar, enquanto um parser aplica uma gramática.** `https://example.com@attacker.test/` tem autoridade `attacker.test` e userinfo `example.com`, e as duas leituras são internamente consistentes.

É por isso que "olhe a URL com atenção" é um conselho fraco. Ele pede que uma pessoa execute uma especificação, em velocidade, sob a suposição de que algo incomum vá parecer incomum — e caracteres homógrafos são escolhidos exatamente para que não pareça.

**A defesa precisa ser mecânica.** Analise a URL, extraia o host, compare com o esperado, e mostre ao usuário o resultado dessa análise em vez da string crua. **Uma máquina lendo uma URL não está sendo cuidadosa; está sendo máquina — que é a propriedade que escala.**
