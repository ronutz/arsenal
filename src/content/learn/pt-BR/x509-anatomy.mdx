...
## Origens: um formato de diretório que virou a camada de identidade da web

O X.509 não foi desenhado para a web. Chegou em 1988 como parte do **X.500**, um sistema de diretório da ITU-T que pretendia dar a cada entidade do mundo um nome distinto numa hierarquia global. Aquele projeto nunca aconteceu, e o formato de certificado sobreviveu a ele.

A herança explica quase tudo o que hoje parece estranho. Nomes distintos com componentes de país, organização e nome comum são entradas de diretório, não identidades web. O **ASN.1** com codificação DER é uma serialização de telecom dos anos 1980, e é por isso que um certificado não é texto legível. E a premissa de que uma hierarquia global nomearia todo mundo é a razão de nada no desenho original dizer qual autoridade pode falar por qual nome - problema remendado depois por [registros CAA e log público](/learn/the-certificate-authority-chokepoint).

As versões registram os reparos:

- **v1 (1988)** - sujeito, emissor, chave, validade. Sem extensões, logo sem como dizer para que servia o certificado.
- **v2** - acrescentou identificadores únicos de emissor e sujeito; quase não foi usada.
- **v3 (1996)** - acrescentou **extensões**, que é onde mora todo o significado moderno: para que a chave pode ser usada, quais nomes ela cobre, onde conferir revogação, se ela pode assinar outros certificados.
- **PKIX (RFC 5280 e sucessoras)** - o perfil de internet que restringe o formato geral a algo interoperável, porque o X.509 sozinho permite muito mais do que a web pode aceitar com segurança.

A web então passou duas décadas corrigindo o modelo de nomes: o nome comum foi usado para nomes de host, depois foi depreciado em favor da extensão **nome alternativo do sujeito**, que é o único campo que um cliente moderno lê para essa finalidade.
...
## Validação, e por que decodificar é a metade fácil

Um certificado analisado diz o que ele afirma. A validação pergunta se a afirmação se sustenta, e é uma sequência sem passos opcionais: montar uma cadeia até uma âncora de confiança que o cliente tenha; verificar cada assinatura; conferir as datas de validade em todos os níveis; confirmar que o nome casa pelo nome alternativo do sujeito; confirmar que uso de chave e uso estendido permitem esta finalidade; impor as restrições básicas para que um certificado de ponta não assine outros; conferir o comprimento do caminho; e consultar a revogação.

**A revogação é o elo fraco.** As listas de certificados revogados ficaram pesadas; o OCSP (Online Certificate Status Protocol) vaza navegação para o emissor e falha de forma leniente quando inalcançável, porque falhar fechado quebraria a web a cada queda de respondedor. O grampeamento move a conferência para o servidor, o must-staple a torna obrigatória para aquele certificado, e a resposta prática da indústria foi vida curta - um certificado que não se consegue revogar de forma confiável ao menos deveria expirar logo.

**A montagem de caminho é onde as implementações divergem.** Raízes com assinatura cruzada significam que mais de uma cadeia válida pode existir, e um cliente que só encontra a expirada falha enquanto outro tem sucesso - causa de várias quedas amplamente sentidas que pareciam expiração e eram diferença de escolha de cadeia.

## O ecossistema, por categoria

- **Autoridades públicas** - emissores comerciais ao lado do Let's Encrypt e de outras organizações automatizadas sem fins lucrativos, todas presas às regras dos programas de raiz dos navegadores e ao log de [Transparência de Certificados](/glossary/certificate-transparency).
- **PKI privada e corporativa** - Microsoft Active Directory Certificate Services, EJBCA, Dogtag, HashiCorp Vault, e emissores nativos de nuvem. Mesmo formato, âncoras de confiança diferentes, e em geral o lugar de onde as quedas por expiração de fato saem.
- **Identidade de carga de trabalho e de vida curta** - SPIFFE/SPIRE, emissores de service mesh, e identidade de instância na nuvem: certificados medidos em horas, emitidos automaticamente, nunca tocados por uma pessoa.
- **Ferramental** - OpenSSL para inspeção e geração, clientes ACME para emissão e renovação, e monitores de logs CT para detectar certificados que você não pediu.
- **Proteção em hardware** - módulos de segurança em hardware e elementos seguros para chaves que não podem ser exportáveis, requisito de qualquer autoridade e cada vez mais de pontas de alto valor.

A regra operacional que sobrevive a qualquer arquitetura: **automatize a renovação ou uma hora tome uma queda**, porque expiração de certificado é a única falha agendada com antecedência que ainda assim surpreende as pessoas.
