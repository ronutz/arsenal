...
Existe um único tipo de objeto cujo corpo não é configuração tmsh. Um `ltm rule` contém uma iRule, que é um programa escrito em Tcl. Seu corpo também usa chaves, mas elas pertencem a estruturas de controle do Tcl como `when { ... }` e `if { ... }`, não à linguagem de configuração. Por isso o corpo de uma iRule deve ser lido na íntegra como código, não interpretado como configurações. O [explicador de configuração tmsh](/tools/tmsh-config-explainer) trata os corpos de iRule exatamente assim: captura-os intactos e nunca tenta executá-los ou reescrevê-los.
