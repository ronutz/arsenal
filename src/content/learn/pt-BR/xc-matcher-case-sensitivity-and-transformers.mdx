...
## O transformador decide o que significa "igual"

Um matcher compara a requisição com um valor, e **os transformadores decidem em que forma os dois lados estão antes dessa comparação.** Erre o transformador e a comparação fica tecnicamente correta sobre dados que já não se parecem com o que chegou.

É aí que mora a evasão. Um caminho comparado sem normalização trata `/Admin`, `/admin` e uma grafia percent-encoded como três strings diferentes — então um DENY sobre uma deixa as outras alcançáveis, e a regra parece certa na revisão. Aplicar os mesmos transformadores que o servidor de origem aplica é o que faz a decisão da política corresponder à requisição que a origem de fato verá.

**A normalização divergente entre política e origem é a brecha**, e ela é invisível de cada lado isoladamente: a política aplica sua regra corretamente, a origem serve a requisição corretamente, e as duas discordam sobre o que a requisição dizia.
