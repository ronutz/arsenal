...
O ponto arquitetural é que a separação é na camada de gerência enquanto a infraestrutura pode ser compartilhada. Isso faz da atribuição de inquilino um controle de segurança, e não uma conveniência organizacional, com o mesmo peso da atribuição de VLAN (rede local virtual) numa malha de switches compartilhada.
...
O FortiEDR expõe uma API de gerência, que importa para três coisas na prática: puxar eventos para um SIEM (security information and event management), automatizar resposta a partir de uma plataforma de orquestração, e reportar sobre uma frota sem clicar pelo console.
