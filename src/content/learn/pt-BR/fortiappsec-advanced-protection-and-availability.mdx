...
Um WAF (web application firewall) inspeciona requisições que chegam à sua aplicação. Três categorias de risco ficam fora disso, e um serviço em nuvem está posicionado para endereçá-las de um jeito que um equipamento no seu rack não está.
...
O **GSLB (global server load balancing)** decide qual região responde, e faz isso por DNS: um resolvedor pergunta onde o nome vive, e a resposta depende de onde o cliente está e de quais regiões estão saudáveis.
