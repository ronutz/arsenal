...
O [explicador de métodos de persistência](/tools/f5-persistence-method-explainer) imprime os modos de falha de cada método ao lado de sua configuração, então um perfil de endereço de origem mostra a ressalva do NAT e um perfil SSL mostra a ressalva do TLS 1.3 diretamente ao lado das configurações. Para o catálogo completo de métodos, veja [métodos de persistência do BIG-IP](/learn/ltm-persistence-methods); para como um primário e um fallback se combinam, veja [fallback de persistência e match-across](/learn/fallback-persistence-and-match-across).
