...
O caminho completo de uma requisição pela plataforma, do qual esta comparação é um passo, está em **[uma requisição, de ponta a ponta](/learn/f5-distributed-cloud-one-request-end-to-end)**.
...
O lado das ações é mais estreito, e isso é deliberado. Uma iRule pode fazer quase qualquer coisa; uma regra de service policy escolhe entre um conjunto fixo de ações e modificadores. ALLOW e DENY são o núcleo. NEXT_POLICY passa para a próxima política do conjunto. Além do veredito, uma regra pode anexar modificadores como uma ação WAF (web application firewall), uma ação de bot ou um rate limiter, que é como você compõe comportamento que uma iRule teria escrito inline.
...
## Escolha por onde a decisão pertence, não pelo que é possível

Quase tudo que uma service policy faz também dá para fazer numa iRule, o que torna capacidade um critério inútil. O critério útil é **quem vai precisar ler a regra daqui a seis meses.**

Uma service policy é declarativa e inspecionável: pode ser listada, comparada, revisada por alguém que não escreve TCL, e raciocinada sem ser executada. Uma iRule é um programa — mais expressiva, e **seu comportamento só é plenamente conhecível executando.**

Então a fronteira não é técnica. **Ponha na política tudo que expresse uma regra que alguém de fora do time possa ter de auditar, e reserve iRules para os casos em que a lógica realmente precisa de um programa.** Um controle de segurança que só uma pessoa consegue ler é um controle com um ponto único de falha que não é a plataforma.
