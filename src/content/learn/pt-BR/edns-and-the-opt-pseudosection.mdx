...
## O tamanho de buffer é uma promessa que a rede pode não cumprir

O registro OPT anuncia o tamanho de resposta UDP que o cliente aceita — normalmente 1232 ou 4096 bytes. **Anunciar não é o mesmo que receber.**

Uma resposta maior que o MTU do caminho é fragmentada, e muitos middleboxes e firewalls descartam fragmentos IP sem avisar. A consulta sai, o servidor responde, a resposta nunca chega, e o resolvedor espera um timeout que vai atribuir ao servidor.

**É por isso que o DNS "estar lento" tantas vezes não é lentidão.** É uma resposta grande falhando em silêncio e um cliente esperando uma retentativa. O indício é que consultas pequenas funcionam perfeitamente e as de DNSSEC ou TXT não — porque são as que crescem.
