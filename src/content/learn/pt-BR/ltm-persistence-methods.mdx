...
Dois métodos existem para protocolos específicos. A persistência **MSRDP** (Protocolo de Área de Trabalho Remota da Microsoft, do inglês Microsoft Remote Desktop Protocol) rastreia sessões de Microsoft Remote Desktop para que um cliente que reconecta retorne ao mesmo host de sessão. A persistência **SIP** (Protocolo de Iniciação de Sessão, do inglês Session Initiation Protocol) baseia-se em um cabeçalho SIP, por padrão o Call-ID, para que todas as mensagens de uma chamada cheguem ao mesmo servidor. Cada um faz sentido apenas para seu protocolo.
