...
A migração via UCS ganha a opção `validate` no `tmsh load sys ucs ... platform-migrate`: uma prévia em JSON do que uma carga mudaria, com avisos para objetos ignorados, e a lógica de ajuste de plataforma que vivia na ferramenta F5 Journeys agora embutida, removendo trunks e configurações de STP (Protocolo Spanning Tree, do inglês Spanning Tree Protocol) não suportados no caminho para rSeries, VELOS ou VE, preservando o IP de gerência e o DHCP (Protocolo de Configuração Dinâmica de Host, do inglês Dynamic Host Configuration Protocol) do destino. Finalmente é possível trocar o administrador primário da conta literal "admin", por TMUI, TMSH ou iControl REST. Uma primeira versão da TMUI modernizada chega desabilitada por padrão atrás da preferência New UI (Beta). E uma remoção merece checagem pré-upgrade: o suporte a Node.js v0.12 acabou, então workspaces de iRules LX em v0.12 precisam migrar para Node.js v6 antes do upgrade, com os workspaces afetados sinalizados nos logs.

## O portão de plataforma, mais uma vez

Nada disso se aplica a iSeries ou VIPRION: o 21.x não os suporta, tentativas de instalação podem falhar no boot, e essas plataformas seguem no 17.x até suas datas de fim de suporte de software. O 21.x roda em rSeries, VELOS e Virtual Edition; um tenant vCMP pode tecnicamente ser implantado com 21.x, mas não é suportado, conforme o [K4309](https://my.f5.com/manage/s/article/K4309). A [visão geral do 21.x](/learn/bigip-21x-whats-new) carrega o quadro completo de ciclo de vida em torno do núcleo operacional deste artigo.

## Fontes

Toda afirmação específica de versão acima é da própria F5, e é aqui que se deve reconferir quando a próxima versão sair.

- [New Features in BIG-IP 21.0.0 - control-plane scale and MCPD efficiency](https://techdocs.f5.com/en-us/bigip-21-0-0/big-ip-release-notes/big-ip-new-features/)
- [What's new in BIG-IP v21.0 - DevCentral](https://community.f5.com/kb/technicalarticles/whats-new-in-big-ip-v21-0/344667)
