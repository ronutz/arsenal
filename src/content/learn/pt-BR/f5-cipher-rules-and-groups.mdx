...
Uma **cipher rule** é um objeto nomeado que guarda uma cipher string, junto com metadados opcionais. O BIG-IP vem com várias rules pré-definidas, e suas strings valem a pena conhecer: `f5-default` é a palavra-chave `DEFAULT`, `f5-ecc` é `ECDHE:ECDHE_ECDSA` (sendo ECDHE o Diffie-Hellman efêmero de curva elíptica, do inglês ephemeral elliptic-curve Diffie-Hellman), e `f5-secure` é `ECDHE:RSA:!SSLV3:!RC4:!EXP:!DES` (sendo RSA o algoritmo Rivest-Shamir-Adleman). Essas rules pré-definidas são somente leitura e servem como modelos de partida.
