...
Há uma regra rígida a conhecer. Se qualquer regra na política é configurada com uma ação de App Firewall (WAF (web application firewall)), o algoritmo de combinação **precisa** ser FIRST_MATCH. Os modos de override não são permitidos junto com ações WAF, porque o processamento WAF está atrelado à avaliação sequencial. Se você vê ações WAF nas regras, pode assumir FIRST_MATCH.
...
## O algoritmo decide quais erros são sobrevivíveis

Ordenação não é só precedência; ela decide **quanto custa uma regra fora de lugar.**

Sob primeira-correspondência, um ALLOW amplo acima de um DENY específico desliga o DENY em silêncio — e a configuração continua contendo os dois, então quem revisa a lista vê a proteção que espera. A regra está presente, correta e inalcançável.

É por isso que **específico antes de geral** não é conselho de estilo. É a ordenação sob a qual um erro fica visível: uma regra específica errada para de casar e é notada, enquanto uma regra geral cedo demais continua funcionando perfeitamente enquanto sombreia tudo abaixo.

**Leia uma política de baixo para cima quando estiver procurando o que NÃO está sendo aplicado.** De cima para baixo você vê o que acontece; de baixo para cima, o que nunca pode acontecer.
