...
Um FortiGate VM é o mesmo FortiOS que você roda num appliance, implantado como máquina virtual em AWS, Azure, GCP ou num hipervisor privado. Você dimensiona, implanta, configura, atualiza e opera. Tudo no resto desta série se aplica sem mudança: políticas, NAT (tradução de endereços de rede), roteamento, perfis de segurança, alta disponibilidade.
...
Uma mesma organização comumente roda os três: VMs protegendo cargas em nuvem, SASE (secure access service edge) cobrindo a força de trabalho, e appliances físicos nos sites que precisam deles. A linguagem de configuração compartilhada entre eles é a razão de a combinação ser administrável.
