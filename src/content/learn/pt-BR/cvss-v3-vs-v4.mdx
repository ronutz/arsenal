...
O CVSS (Sistema Comum de Pontuação de Vulnerabilidades, do inglês Common Vulnerability Scoring System) teve várias versões, e um vetor só significa algo no contexto da sua versão. O [decodificador de vetores CVSS](/tools/cvss-vector-decoder) calcula **v3.0 e v3.1**, que são as versões que a grande maioria dos comunicados publicados e dos feeds de vulnerabilidade ainda usa.
...
## Por que o prefixo de versão não é enfeite

Todo vetor começa pela versão — `CVSS:3.1/...` — e esse prefixo sustenta o resto. **As mesmas letras de métrica significam coisas diferentes entre versões**, então um vetor lido sob a versão errada produz um número que parece plausível e está errado.

A v4 é o caso mais claro: renomeia a pontuação base para **CVSS-B** e acrescenta os grupos Threat e Supplemental, justamente porque a indústria passou uma década tratando uma pontuação base como uma declaração completa de risco. Uma ferramenta que ignora o prefixo não está sendo tolerante; está adivinhando.

**Quando copiar um vetor de um aviso, copie o prefixo junto.**
