...
## A forma canônica existe porque igualdade é mais difícil do que parece

Dois documentos JSON podem ser idênticos em significado e diferentes em bytes: ordem de chaves, espaços, formatação de números, escapes de string. **Cada uma dessas diferenças é invisível para um programa que lê o valor analisado e fatal para qualquer coisa que faça hash ou assine o texto.**

É essa a razão de existir canonicalização, e a razão de assinar JSON ser mais sutil do que parece. **Uma assinatura cobre bytes; uma aplicação compara valores.** Reserializar um documento — mesmo por uma biblioteca que produz JSON perfeitamente correto — pode quebrar uma assinatura válida um instante antes, sem erro em lugar nenhum até a verificação falhar em outro lugar.

**Assine os bytes que você recebeu, ou canonicalize deliberadamente antes de assinar.** Qualquer coisa no meio é uma assinatura que funciona até uma dependência atualizar o serializador.
