...
## Um predicado que nunca casa é idêntico a um que sempre permite

Os dois níveis de lógica — E entre predicados, OU dentro de um matcher — significam que uma regra pode ficar **mais estreita do que se pretendia sem falhar.** Um nome de cabeçalho escrito errado, um prefixo de caminho sem a barra inicial, um seletor cujo rótulo não existe mais: cada um torna o predicado insatisfazível, e um predicado insatisfazível numa regra ALLOW só significa que a regra nunca dispara.

O tráfego então cai para o que vier depois. **A política está aplicando alguma coisa, então nada parece quebrado** — e se o que vem depois for permissivo, a regra que você escreveu para restringir a requisição não contribui em nada, embora apareça na configuração.

**Teste que uma regra casa o que deve, e também que ela casa alguma coisa.** Uma regra com zero acertos depois de um dia de tráfego ou é desnecessária ou está errada, e a configuração não diz qual.
