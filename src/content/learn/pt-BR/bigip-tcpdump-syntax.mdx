...
## A interface que você nomeia muda o que dá para ver

Nomear `0.0` captura através do TMM, o que normalmente é o certo. Mas a escolha não é só sobre escopo, é sobre **que estágio do processamento você observa.**

O tráfego num proxy completo existe em duas formas — lado cliente e lado servidor — e são conexões diferentes, com endereços e portas diferentes. Um filtro escrito para o endereço do cliente não casa com o fluxo do lado servidor, e quem conclui "a requisição nunca chegou ao membro do pool" a partir desse filtro não provou nada disso.

**Capture os dois lados, ou capture com um filtro largo o bastante para conter ambos, e estreite depois na leitura.** Sai mais barato descartar pacotes no Wireshark do que repetir uma captura durante um incidente porque o filtro foi esperto demais.
