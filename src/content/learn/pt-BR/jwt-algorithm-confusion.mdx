...
## O verificador decide o algoritmo, não o token

Os dois ataques clássicos são o mesmo erro: **deixar o cabeçalho do próprio token escolher como ele é conferido.** `alg: none` pede que não haja verificação, e trocar `RS256` por `HS256` pede que o verificador use uma chave pública como segredo HMAC — uma chave que o atacante também tem, porque é pública.

Isso continua acontecendo porque a API conveniente é a insegura. Um `verify(token, chave)` que lê o algoritmo do cabeçalho está fazendo o que o token pediu. **Uma chamada segura declara o algoritmo esperado e rejeita qualquer outro antes de olhar a assinatura.**

O princípio geral transcende o JWT: **uma mensagem nunca pode escolher os termos sob os quais é julgada.**
