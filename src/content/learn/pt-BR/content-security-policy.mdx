...
Defina o `default-src` primeiro e depois abra tipos específicos só quando necessário. Uma política que parte de `default-src 'self'` e adiciona uma CDN (rede de entrega de conteúdo, do inglês content delivery network) conhecida ao `script-src` é muito mais fácil de raciocinar do que uma com uma dezena de diretivas e nenhum fallback.
