...
Uma resposta de resolução de nomes do BIG-IP DNS (antigo GTM - Global Traffic Manager) é decidida duas vezes. O wide IP recebe a consulta e seleciona um de seus pools; o pool selecionado então seleciona um membro, um virtual server, cujo endereço vira a resposta. As duas camadas usam conjuntos de métodos diferentes e regras diferentes, e a maioria das surpresas de GSLB (balanceamento de carga global de servidores, do inglês global server load balancing) mora na segunda.
