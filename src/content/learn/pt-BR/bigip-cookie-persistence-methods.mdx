...
## A persistência sobrevive ao membro do pool

Um registro de persistência é uma promessa de continuar mandando o cliente para o mesmo membro, e é mais forte do que se espera: **um membro marcado como fora do ar não libera automaticamente os clientes fixados nele.**

O que acontece em seguida depende de configuração, não de intuição. A conexão pode ser rebalanceada ou pode ser recusada, e a diferença entre esses dois desfechos durante um incidente é a diferença entre serviço degradado e indisponibilidade para um subconjunto de usuários que não entende por que todos os outros estão bem.

**Teste a falha, não o recurso.** Persistência funcionando é fácil de observar; persistência liberando corretamente quando um membro cai é o comportamento que importa, e é o que ninguém verifica até virar carga.
