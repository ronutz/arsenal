...
## Sem o fluxo par, você tem duas capturas de um problema

Os sufixos de detalhe acrescentam o trailer da F5, e o campo que muda o que uma captura consegue provar é o **identificador de fluxo par**. É ele que afirma que esta conexão do lado cliente e aquela do lado servidor são duas metades de uma transação.

Sem ele, correlacionar é adivinhação vestida de análise: casar por tempo, por fragmentos de payload, pela suposição de que nada mais estava acontecendo. **Num virtual server movimentado essa suposição é simplesmente falsa**, e um par errado produz uma conclusão confiante sobre uma transação que nunca existiu.

**Capture com detalhe suficiente para correlacionar, ou aceite que está analisando duas conversas sem relação.** A posição intermediária — uma captura com os dois lados e sem como pareá-los — é a que mais custa tempo, porque parece completa.
