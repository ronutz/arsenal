...
## A equação não é o argumento

As fórmulas são públicas e reproduzíveis, o que faz o CVSS parecer objetivo de um jeito que não é. **Todo número vem de valores de métrica que alguém selecionou**, e a seleção é julgamento: exige interação do usuário, o escopo muda, a complexidade de ataque é baixa.

Dois analistas pontuando a mesma falha honestamente podem divergir mais que uma faixa de severidade, e **o vetor é a única parte de uma pontuação publicada com a qual dá para discutir.** Um número solto não pode ser conferido; um vetor pode ser lido, contestado e repontuado.

**Publique o vetor, sempre, e trate uma pontuação sem ele como opinião vestida de número.** A precisão da equação é real e é uma precisão sobre as entradas, não sobre a vulnerabilidade.
