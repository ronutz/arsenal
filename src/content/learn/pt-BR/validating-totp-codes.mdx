...
## Um código correto continua correto pelo resto do passo

A verificação responde *"este é o código certo para este momento"*, e pelo restante daquele passo de tempo a resposta continua sendo sim. **A menos que o validador registre que um código foi usado, o mesmo código funciona de novo.**

É isso o replay de TOTP inteiro, e não exige criptografia para explorar — um código visto por cima do ombro, lido de uma captura de tela ou registrado num log de proxy é utilizável até o passo virar.

**Guarde o último passo aceito por usuário e recuse qualquer coisa igual ou anterior.** É um inteiro, e converte um código válido por trinta segundos em um código válido uma vez.

**Nada da omissão aparece em teste**, porque uma implementação correta e uma reproduzível se comportam igual para um usuário que nunca reusa um código.
