...
relatedArticles: ["zscaler-nanolog-nss-and-log-streaming", "zia-cloud-firewall-rule-order", "zia-url-filtering-and-cloud-app-control", "syslog-message-formats", "zscaler-exfiltration-response-posture"]
category: security
order: 193
status: reviewed
updated: "2026-07-21"
...
Um log é uma frase, e frases são feitas de vocabulário. No Zscaler Internet Access (ZIA), esse vocabulário é literal: um feed de log é um **template de campos que você compõe** - `%s{login}`, `%s{csip}`, `%s{action}` - e o feed emite exatamente a frase que você projetou, registro após registro, [transmitida para onde quer que o seu SIEM (security information and event management) more](/learn/zscaler-nanolog-nss-and-log-streaming). Este artigo é sobre o vocabulário em si: quais campos carregam quais respostas, e as três sutilezas documentadas que derrubam projetistas de feed de primeira viagem. Fundamentado na documentação de formatos de feed NSS da Zscaler, verificada em 2026-07-21.
