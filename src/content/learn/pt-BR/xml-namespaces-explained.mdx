...
## O prefixo é local e a URI é a identidade

Um prefixo de namespace não significa nada sozinho. `soap:Envelope` e `s:Envelope` são o mesmo elemento se os dois prefixos estiverem ligados à mesma URI, e elementos diferentes se não estiverem — **o prefixo é uma abreviação local, e a URI é o que de fato identifica o nome.**

Isso produz uma classe específica de falha silenciosa. **Uma expressão XPath que casa por prefixo casa texto, não identidade**, então um documento que religa prefixos — de forma perfeitamente legal — escapa de um seletor que parecia funcionar em todos os testes. Verificação de assinatura e casamento de política são onde isso mais dói, porque as duas decidem algo importante com base em "isto selecionou o elemento que eu queria".

**Ligue prefixos explicitamente na consulta e compare URIs, não prefixos.**
