...
## O parser foi configurado anos antes do seu código rodar

XXE raramente é um bug que alguém escreveu. É um **padrão** que alguém herdou: um parser com resolução de entidades ligada porque essa era a escolha sensata quando a biblioteca foi projetada, numa época em que documentos XML vinham de colegas, não da internet.

É por isso que a correção quase nunca está no código que trata o documento e quase sempre na construção do parser — e por que uma aplicação pode ser vulnerável numa implantação e segura em outra **com código-fonte idêntico**, porque a plataforma trouxe outro padrão.

**Ache onde o parser é construído, não onde o documento é lido.** E confira depois de cada atualização de dependência: um padrão que mudou a seu favor pode mudar de volta, e nada num changelog é endereçado a uma premissa de segurança que você nunca escreveu.
