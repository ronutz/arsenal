...
relatedArticles: ["f5xc-origin-pool-anatomy", "f5xc-domain-matching-and-listener-logic", "f5xc-lb-algorithms-and-persistence"]
category: networking
order: 72
status: reviewed
updated: "2026-07-11"
...
When you bring up a CE (Customer Edge node), the first thing it does is call home. Using the node token you generated - a JWT that encodes the registration endpoint - the CE contacts the F5 Distributed Cloud Global Controller. The controller selects two Regional Edges based on the GeoIP proximity of the public IP address the registration arrived from, so the CE connects to the REs closest to where it actually sits on the internet. (You can also pin specific REs at creation time.) The controller returns those two REs to the CE, along with the CE's identity and PKI (public key infrastructure) certificates, which the system rotates regularly.
