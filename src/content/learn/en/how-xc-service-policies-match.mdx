...
## Matching order is where a policy becomes auditable or does not

A request is evaluated against rules in an order, and **the order is the part a reader has to reconstruct in their head** — which is exactly where review fails. A policy that is correct but requires simulation to understand will be maintained by guesswork.

That gives a practical test for any rule you add: **can somebody reading the list, top to bottom, predict what this does to a request?** If the answer needs an execution trace, the policy is doing something a policy is the wrong tool for.

**Keep the number of rules that can match a single request small.** Not for performance — for the far more expensive property that somebody will need to change this under pressure and be right the first time.
