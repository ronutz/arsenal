...
An F5 Distributed Cloud (XC) **service_policy** decides which client requests to an application are allowed and which are denied. If you come from BIG-IP, it sits in roughly the place an access policy or a set of iRules would, but its shape is declarative rather than procedural.
