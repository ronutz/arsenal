...
Set `default-src` first, then open up specific types only as needed. A policy that starts from `default-src 'self'` and adds a known CDN (content delivery network) to `script-src` is far easier to reason about than one that lists a dozen directives with no fallback.
