...
relatedArticles: ["json-numbers-and-precision", "json-duplicate-keys", "url-anatomy", "json-comments-and-trailing-commas"]
category: encoding
order: 1
status: reviewed
updated: "2026-06-29"
...
JSON was designed to be the smallest data format that is still useful, and that smallness is the whole point: it is easy to write a parser, easy to read, and hard to disagree about. The grammar is defined identically by RFC 8259 and ECMA (European Computer Manufacturers Association)-404, and it fits on a postcard. A JSON document is exactly one value, and a value is one of six things: an object, an array, a string, a number, a boolean, or null. Everything else is built from those.
