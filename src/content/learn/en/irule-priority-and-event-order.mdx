...
concepts: ["irules", "priority", "event order", "execution order", "big-ip", "f5"]
...
The mental model that keeps it straight: **event order is across the connection; priority is within one event.** [iRules event-order explainer](/tools/f5-irules-event-order) shows the first; priority controls the second, and only matters when you have multiple handlers competing for the same moment.
