...
A resolver changed by DHCP, a VPN that installed its own, a container inheriting a different `resolv.conf`: each produces answers that are correct for a resolver nobody meant to consult. **The answer is not wrong; the question went somewhere unexpected**, and the output looks entirely normal.
