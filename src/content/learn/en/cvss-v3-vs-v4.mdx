...
CVSS (Common Vulnerability Scoring System) has had several versions, and a vector only means something in the context of its version. The [CVSS vector decoder](/tools/cvss-vector-decoder) computes **v3.0 and v3.1**, which are the versions the great majority of published advisories and vulnerability feeds still use.
...
## Why a version prefix is not decoration

Every vector begins with its version — `CVSS:3.1/...` — and that prefix is load-bearing. **The same metric letters mean different things across versions**, so a vector read under the wrong version produces a number that looks plausible and is wrong.

v4 is the clearest case: it renames the base score to **CVSS-B** and adds Threat and Supplemental groups, precisely because the industry had spent a decade treating a base score as a complete risk statement. A tool that strips or ignores the prefix is not being lenient; it is guessing.

**When you copy a vector out of an advisory, copy the prefix with it.**
