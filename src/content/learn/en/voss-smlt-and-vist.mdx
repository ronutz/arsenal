...
concepts: ["extreme", "smlt", "vist", "virtual ist", "rsmlt", "slpp", "mlt", "smlt-virtual-bmac", "dual-homing"]
...
Traditionally the IST is a dedicated physical link between the two cluster switches. **Virtual IST (vIST)** runs that trunk as a channel through the SPBM fabric instead, so no direct cable between the peers is required. You create a vIST VLAN with an I-SID (Individual Service Identifier) and a Layer 3 interface, then point the peers at each other:
