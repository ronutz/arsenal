...
- **Base32** uses `A`-`Z` and `2`-`7`, case-insensitive, with the confusable characters `0`, `1`, and `8` removed. It is built for values a human will type, dictate, or store case-insensitively, like TOTP (time-based one-time password) secrets and onion addresses.
