...
## The capture itself is a change

`tcpdump` on a production BIG-IP is not a read-only operation in any sense that matters operationally.

**It writes.** A full-size capture on a busy virtual server fills a filesystem quickly, and a full `/var` on a BIG-IP is not a disk-space inconvenience — it is a device that starts failing in ways unrelated to the problem you were capturing.

**It costs.** Capture processing competes with traffic processing, and the more permissive the filter, the more it competes.

So the discipline is the opposite of instinct: **filter tightly, bound the capture, and know how it stops before you start it.** A size or packet-count limit and a specific host-and-port filter turn an open-ended risk into a bounded one. A capture that has to be killed by an engineer who is also handling an incident is a capture that was planned badly.
