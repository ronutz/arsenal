...
## The PKCS#10 structure, field by field

Underneath that three-part summary, PKCS#10 defines a precise ASN.1 shape. The outer `CertificationRequest` is a `SEQUENCE` of exactly three things: the request information, the signature algorithm, and the signature itself.
...
CertificationRequestInfo ::= SEQUENCE {
  version        INTEGER,                -- always v1 (0)
  subject        Name,                   -- the distinguished name you request
  subjectPKInfo  SubjectPublicKeyInfo,   -- algorithm + your public key bits
  attributes     [0] Attributes }        -- everything else you ask for
```

The `certificationRequestInfo` is the "to-be-signed" half: your subject name, your public key wrapped in the very same `SubjectPublicKeyInfo` structure that will later appear in the finished certificate (see the [anatomy article](/learn/x509-anatomy)), and an `attributes` set. The signature is computed over the DER encoding of that half using your private key, which is what mechanically proves possession of the key described above.

The `attributes` set is where the interesting requests live. A modern certificate identifies a server by its Subject Alternative Names, not its subject common name, and those SANs travel here inside the PKCS#9 `extensionRequest` attribute, which is a container for the X.509v3 extensions you would like the CA to copy into the certificate. When the [CSR decoder](/tools/csr-decoder) shows you requested SANs or key usages, it is reading them out of this attribute. (PKCS#10 version 1.7, in RFC 2986, obsoletes the older version 1.5 from RFC 2314; the structure above is what every current tool emits.)
...
If you sign your own CSR with your own key instead of sending it to a CA, you get a self-signed certificate, where the Issuer equals the Subject. It carries no external authority, because the only thing vouching for it is itself, but it is perfectly useful for local development, internal services with their own trust store, or acting as the root of a private PKI (public key infrastructure). The tool flags when a certificate is self-issued, which tells you immediately that its trust comes from somewhere other than a public CA.
