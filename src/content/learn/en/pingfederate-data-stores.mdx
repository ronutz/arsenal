...
concepts: ["ping", "pingfederate", "data stores", "ldap", "jdbc", "failover", "validation query", "attribute lookup"]
relatedTools: []
relatedArticles: ["ldap-fundamentals", "scim-overview", "pingfederate-authentication-adapters", "pingfederate-admin-access-and-rbac", "pingfederate-authentication-policies", "ognl-injection-in-waf-logs"]
category: identity
order: 39
status: reviewed
updated: "2026-07-20"
...
A data store in PingFederate is a **connection definition**: how to reach an external source of identity data, configured once and referenced everywhere. The consumers are the interesting part - Password Credential Validators verify passwords against a store, attribute mappings look up additional attributes from a store during SSO (single sign-on), and provisioning reads and writes through a store. Change the directory's address or rotate the service credential, and you change it in exactly one place; every consumer follows. That indirection is the design, and recognizing it is what the objective's "configure **or manage**" is really asking.
