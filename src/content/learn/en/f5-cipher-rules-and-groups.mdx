...
A **cipher rule** is a named object that holds a cipher string, along with optional metadata. BIG-IP ships several pre-built rules, and their strings are worth knowing: `f5-default` is the keyword `DEFAULT`, `f5-ecc` is `ECDHE:ECDHE_ECDSA` (ECDHE being ephemeral elliptic-curve Diffie-Hellman), and `f5-secure` is `ECDHE:RSA:!SSLV3:!RC4:!EXP:!DES` (RSA being the Rivest-Shamir-Adleman algorithm). These pre-built rules are read-only and meant as starting templates.
