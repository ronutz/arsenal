...
concepts: ["big-ip", "cookie persistence", "insert", "httponly", "secure", "f5"]
...
## Persistence outlives the pool member

A persistence record is a promise to keep sending a client to the same member, and it is stronger than most people expect: **a member marked down does not automatically release the clients pinned to it.**

What happens next depends on configuration rather than intuition. The connection may be re-balanced, or it may be refused, and the difference between those two outcomes during an incident is the difference between a degraded service and an outage for a subset of users who cannot understand why everyone else is fine.

**Test the failure, not the feature.** Persistence working is easy to observe; persistence releasing correctly when a member fails is the behaviour that matters, and it is the one nobody checks until it is load-bearing.
