...
A **deployment network** is a segment where decoys are placed. In practice this means a VLAN, and the decoys take addresses on it as ordinary hosts would.
...
Sending alerts to **FortiAnalyzer or a SIEM** puts them beside everything else, so a decoy interaction can be correlated with what the same source did elsewhere. That correlation frequently converts one alert into a timeline.
...
The caution that remains is the compromised-shared-address case. Quarantining a source that turns out to be a NAT gateway or a jump host affects everyone behind it, so the automation should account for what sits behind the address it is about to cut off.
