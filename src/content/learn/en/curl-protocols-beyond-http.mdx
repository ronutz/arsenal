...
A historical footnote: older curl builds also listed the RTMP (Real-Time Messaging Protocol) streaming family. It lives on only in builds linked against librtmp and is absent from the current official list, which is why tools targeting current curl treat it as legacy.
