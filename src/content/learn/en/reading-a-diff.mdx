...
The [diff viewer](/tools/diff) shows a **unified** view: one column with removed and added lines interleaved in place, which reads well in a narrow space and keeps changes next to their context. The other common layout is **side-by-side**, with the original on the left and the changed version on the right. Classic command-line diff also groups changes into **hunks**, each introduced by an `@@` header and surrounded by a few unchanged "context" lines, so a patch can be applied to a file even after it has shifted. The aligned view here shows the full text rather than separate hunks.
