...
## TLS 1.3 does not read your 1.2 cipher string

The most common surprise here is not about ordering at all. **TLS 1.3 defines its own suites**, and they are negotiated separately from the TLS 1.2 and earlier list.

A carefully restrictive cipher string, written to exclude weak 1.2 suites, can leave TLS 1.3 entirely unaffected — the connection negotiates a 1.3 suite that the string never mentioned, and a compliance scan reports something the configuration appears to forbid.

**Check which protocol version a test connection actually negotiated before concluding a cipher rule worked.** A rule that governs 1.2 and a client that speaks 1.3 will agree on nothing and produce no error, which is the quietest way for a security control to be absent.
