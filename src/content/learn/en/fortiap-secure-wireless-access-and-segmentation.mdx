...
**Enterprise (802.1X)** authenticates each user or device individually against RADIUS. Credentials are per identity, revocation is per identity, and the RADIUS server can return attributes describing what this identity should get. This is what makes the segmentation below possible.
...
The better approach is **dynamic VLAN assignment**. One enterprise SSID, and the RADIUS server returns the VLAN with the authentication result, so staff, contractors and lab devices land in different segments while seeing the same network name.
