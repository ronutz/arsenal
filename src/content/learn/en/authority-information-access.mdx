...
Authority Information Access is a certificate extension, defined in RFC 5280, that tells a relying party where to find services and data related to the issuing CA (certificate authority). It is a list of access descriptions, and each one pairs an access method (an OID that says what kind of pointer this is) with a location (almost always an HTTP URL). The X.509 inspector extracts these and shows them, because between them they cover two very different jobs that are easy to mix up.

## OCSP: where to ask about revocation

The first access method is OCSP, the Online Certificate Status Protocol. Its URL points to the CA's OCSP responder, a service that answers a simple question about one certificate: good, revoked, or unknown. A client that wants to check revocation in real time takes the OCSP URL from this extension, sends the certificate's serial number to that responder, and reads the signed answer. This is the live alternative to downloading a whole CRL (certificate revocation list), and it is covered in depth in the [revocation article](/learn/certificate-revocation). The weakness of querying the responder directly, and the extension that hardens it, is the subject of [OCSP Must-Staple](/learn/ocsp-must-staple).
