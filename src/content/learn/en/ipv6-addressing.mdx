...
IPv4 has roughly 4.3 billion addresses (2^32). That seemed inexhaustible in the 1980s and is now long exhausted at the registries. IPv6, defined for addressing in RFC 4291, widens the address from 32 bits to **128 bits**, which is 2^128 addresses, a number large enough that scarcity stops being the design constraint. The practical effect is that subnets become enormous and address-conservation tricks like NAT (network address translation) are no longer required for basic connectivity.
