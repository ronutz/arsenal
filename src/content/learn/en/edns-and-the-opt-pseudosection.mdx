...
A response larger than the path MTU gets fragmented, and plenty of middleboxes and firewalls drop IP fragments without comment. The query goes out, the server answers, the answer never arrives, and the resolver waits for a timeout it will blame on the server.

**This is why DNS "being slow" is so often not slow at all.** It is a large answer failing silently and a client waiting out a retry. The tell is that small queries work perfectly and DNSSEC or TXT lookups do not, because those are the ones that grow.
