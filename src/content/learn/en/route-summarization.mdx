...
Summarization lives at boundaries. In OSPF it is configured on area border routers and AS (autonomous system) boundary routers, since within an area the full link-state topology is flooded and cannot be summarized. EIGRP (Enhanced Interior Gateway Routing Protocol) can summarize on almost any interface. In BGP, aggregation combines specific prefixes into a covering route before advertising them to other autonomous systems, which is a large part of why the global routing table is far smaller than the number of allocated networks.
