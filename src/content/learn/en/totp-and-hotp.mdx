...
Because the offset is derived from the MAC (message authentication code), which byte window becomes the code is itself unpredictable, and every code remains a deterministic function of the secret and counter.
...
## The acceptance window is a brute-force multiplier

Clocks drift, so validators accept codes from adjacent time steps. **Every step you accept multiplies the number of codes valid at any instant**, and a window of ±1 step means three valid six-digit codes rather than one.

That is a threefold improvement in an attacker's odds per attempt, permanently, in exchange for tolerating drift that a well-run client rarely has. A generous window feels like kindness to users and reads, from the other side, as a discount.

**Keep the window at ±1 step and put the real defence on attempt rate.** Guessing six digits is only hard while guessing is slow — **the code length is doing much less work than the lockout is.**
