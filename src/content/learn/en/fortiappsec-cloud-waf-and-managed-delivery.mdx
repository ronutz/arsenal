...
A cloud WAF (web application firewall) is the same protective idea as an on-premises one with a different delivery model, and the differences are what determine whether a deployment works.
...
**DDoS (distributed denial) mitigation** at network and application layers, with capacity that is the point of the model.
...
**Log export** into your own SIEM (security information and event management), so cloud WAF events sit alongside everything else rather than in a separate console that gets checked after an incident rather than during one.
