...
## The trap: nslookup does not use the system resolver

`nslookup` talks to a DNS server directly. It does **not** consult `/etc/hosts`, and on most systems it bypasses the resolver library that ordinary applications use.

So a name can resolve perfectly for `ping` and fail in `nslookup`, or the reverse — and neither result is wrong. They are answering different questions: nslookup answers *"what does DNS say"*, while the application answers *"what does this host resolve to, by every mechanism available to it"*.

**When a service works and nslookup disagrees, check the hosts file before you check the zone.** It is the most common reason for two tools telling you opposite things about the same name.
