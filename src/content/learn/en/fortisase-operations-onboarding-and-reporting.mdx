...
A SASE (secure access service edge) service only protects the traffic that reaches it. Everything operational follows from that, starting with how users get on.
...
Where SASE carries traffic that has performance requirements, service-level agreements measure the paths — latency, jitter, packet loss — and steer traffic accordingly. This is the same mechanic as SD-WAN (software-defined WAN) performance SLA (service level agreement), applied to the path between the user and the point of presence.
