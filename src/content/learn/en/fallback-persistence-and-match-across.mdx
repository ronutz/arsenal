...
Because these settings interact, reading them by eye across several profiles and virtuals is error-prone. The [persistence-method explainer](/tools/persistence-method-explainer) lays out each virtual's primary and fallback together with the notes that apply, and shows the match-across settings on each profile with what they do. For the methods themselves, see [BIG-IP persistence methods](/learn/ltm-persistence-methods); for choosing among them, see [choosing a persistence method](/learn/choosing-a-persistence-method).
