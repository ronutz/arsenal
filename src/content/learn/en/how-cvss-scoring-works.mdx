...
Because every step is deterministic, pasting a vector into a calculator should reproduce the vendor's or NVD's (National Vulnerability Database) published score exactly. If it does not, the vector was transcribed wrong, or the two tools disagree on the CVSS version.


## The equation is not the argument

The formulas are public and reproducible, which makes CVSS look objective in a way it is not. **Every number comes from metric values somebody selected**, and the selection is a judgement: is user interaction required, is the scope changed, is the attack complexity low.

Two analysts scoring the same flaw honestly can differ by more than a severity band, and **the vector string is the only part of a published score that can be argued with.** A bare number cannot be checked; a vector can be read, disagreed with, and re-scored.

**Publish the vector, always, and treat a score without one as an opinion in numeric clothing.** The equation's precision is real and it is a precision about the inputs, not about the vulnerability.
