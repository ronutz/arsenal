...
The full request path through the platform, of which this comparison is one step, is laid out in **[one request, end to end](/learn/f5-distributed-cloud-one-request-end-to-end)**.
...
The action side is narrower and that is deliberate. An iRule can do almost anything; a service policy rule chooses among a fixed set of actions and modifiers. ALLOW and DENY are the core. NEXT_POLICY hands off to the next policy in the set. Beyond the verdict, a rule can attach modifiers such as a WAF (web application firewall) action, a bot action, or a rate limiter, which is how you compose behavior that an iRule would have scripted inline.
...
## Choose by where the decision belongs, not by what is possible

Almost anything a service policy does can also be done in an iRule, which makes capability a useless criterion. The useful one is **who needs to read the rule six months from now.**

A service policy is declarative and inspectable: it can be listed, diffed, reviewed by somebody who does not write TCL, and reasoned about without executing it. An iRule is a program — more expressive, and **its behaviour is only fully knowable by running it.**

So the boundary is not technical. **Put in a policy anything that expresses a rule somebody outside the team might have to audit, and reserve iRules for the cases where the logic genuinely needs a program.** A security control that only one person can read is a control with a single point of failure that is not the platform.
