...
Authentication runs against a RADIUS (Remote Authentication Dial-In User Service) server, and the server can return the VLAN (virtual local area network) the device should be placed in. That is how a single physical port serves different network segments depending on who connects to it, and it is the mechanism network access control builds on.
