...
The claims above are not assertions; each is drawn from a case written up in this catalogue. Two companion pages collect them so they can be read as a set rather than found one at a time: **[how systems fail](/learn/how-systems-fail)** sorts the incidents by where the fault actually lived - a removed safeguard, an expired assumption, a boundary, a rollback, an unread alert - and **[the disclosure record](/learn/the-disclosure-record)** collects the cases where the interval between knowing and saying decided the outcome.
