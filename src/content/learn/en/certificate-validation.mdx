...
The [anatomy article](/learn/x509-anatomy) ends on a point worth repeating: decoding a certificate tells you what it *claims*, not whether the claim is *true*. Validation is the separate process a TLS client runs to decide whether to trust the certificate in front of it. It is governed mainly by RFC 5280, and it is a conjunction of independent checks, every one of which must pass. A decoder, including the [X.509 Certificate Decoder](/tools/x509), shows you the inputs to that process; the client is what enforces it.
...
Finally, a certificate that was valid when issued may have been revoked since, for instance because its private key leaked. The client may consult a CRL (Certificate Revocation List) or an OCSP (Online Certificate Status Protocol) responder to find out. The inspector on this page extracts the revocation endpoints the certificate advertises, its CRL distribution points and its OCSP responder, so you can see how a client would check it; it reads those pointers but never contacts them. In practice this is the weakest and most inconsistent step, and the industry is moving toward short-lived certificates instead, which the [revocation article](/learn/certificate-revocation) covers in full.
