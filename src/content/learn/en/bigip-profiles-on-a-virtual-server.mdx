...
concepts: ["big-ip", "profiles", "virtual server", "ssl offload", "inheritance", "f5"]
...
## Where the surprises live: inheritance and order

Two things about profiles cause more confusion than the profiles themselves.

**Profiles inherit from a parent.** A custom profile carries values from the one it was derived from, so a setting nobody on the team ever chose can be in force because it was the parent's default at the time the child was created. **Reading a profile's own page shows you what was overridden, not what is in effect.**

**And the attachment order is not arbitrary.** Profiles line up with the layers of the connection, and a profile that expects to see decrypted content cannot do its job if nothing is decrypting. This is why adding an HTTP profile to a virtual server carrying TLS traffic with no client SSL profile produces no useful result and no obvious error: the layer it wanted to operate on never appeared.
