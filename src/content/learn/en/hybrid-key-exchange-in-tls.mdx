...
The obvious move would be to drop ECDH and use ML-KEM alone. The industry deliberately did not do that, for one reason: ML-KEM is new. Lattice hardness has been studied for years, but a scheme that has only recently been standardized is a younger attack surface than X25519, which has decades of scrutiny behind it. And the field has a fresh memory of what young assumptions can do, the isogeny scheme SIKE (Supersingular Isogeny Key Encapsulation) was broken in 2022 by a classical attack, after years as a serious candidate. Betting the entire key exchange on ML-KEM alone would mean that a surprise cryptanalytic result against it breaks everything.
