...
An access policy pairs a **condition** with a **result**. The condition matches on what FortiNAC knows: the endpoint's classification, its registration state, the user's group membership, the location, the time. The result is the network access to grant — typically a VLAN.
